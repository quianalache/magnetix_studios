...
- **Conversations tab is INTERIM** ([contact-conversations-panel.tsx](src/components/contacts/contact-conversations-panel.tsx)): reuses the profile's existing SMS + WhatsApp thread components and links to the inbox, because the Conversations redesign (`codex/conversations-redesign`) wasn't integrated. After it merges: embed its thread/composer/availability here and remove the header Email/SMS dialogs + standalone thread cards.
...
- **Activity** — `GET /api/contacts/[id]/activity` (paginated, category filter from [activity-categories.ts](src/lib/contacts/activity-categories.ts)); notes appear only as content-free "Note added" markers synthesized from note metadata. Labels in [activity-visuals.tsx](src/components/contacts/activity-visuals.tsx) (AI reply/escalate/skip, watchdog flag and opt-out rows no longer render as "Note added"). New types `purchase_completed`, `course_enrolled`, `community_access_granted`, `community_access_revoked` are written by [contact-activity.ts](src/lib/server/contact-activity.ts) from the existing purchase / enrollment / membership services (deterministic ids where retries could duplicate). Rules: activities read-only for clients. The legacy `activity-timeline.tsx` (still used by main's Conversations panel) is untouched pending the Conversations merge.
- **Purchases & Access** ([contact-access-service.ts](src/lib/server/contact-access-service.ts)) — read projection of purchases / enrollments / memberships. Admins can grant complimentary access to any sub-account community (membership `origin: "staff"` + a `staff:contact-grant` access source recording the prior state), open standalone courses and free offers (existing enrollment paths). Revoke is community-only and never removes access a paid group purchase or an active product source still provides. Paid courses/offers are NOT grantable: their guards require a paid purchase record and fake purchases are disallowed. A contact without a Member gets one created linked to THAT contact (never adopting another contact's member).
