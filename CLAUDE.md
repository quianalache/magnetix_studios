...
| `metaAgentEnabledByAgency` | **Messenger & Instagram AI auto-reply** (the "meta" AI channel) — the shared agent answering inbound FB/IG DMs. Layered ON TOP of the inbox gate: inbox makes the DMs exist, this lets the AI answer them (spends OpenRouter credits). Enable-time 403 at the channels route; runtime: the Meta inbound webhook skips AI dispatch. Grayed out while Meta creds are unset, like the other Meta gates | `false` (opt-IN, WhatsApp precedent) | No — channel config + token counter preserved; manual inbox replies unaffected. Re-enable resumes instantly. In `PLAN_GATE_KEYS` (not pre-ticked in new plans). |
...
src/
  app/
    ...
    api/
      ...
      cron/gitpage-heartbeat/     Daily QStash-scheduled gitpage subscription-status refresh
  ...
  lib/
    ...
    gitpage/             client.ts (gitpage REST SDK) + heartbeat.ts (subscription status cache)
...
instrumentation.ts       Cold-start gitpage status ping
...
| `system/heartbeat` | server-only | Instance id + last-heartbeat timestamp (gitpage status checks) |
...
## Workflow Builder (v1 + v2)

The general automation engine + visual builder at `/sa/[id]/workflows` (it replaced the legacy single-recipe `automations` engine — those files are gone; Speed-to-Lead lives on as a starter template). A workflow = a **trigger** + a graph of **nodes** (linear steps with two-way branches); each contact enrollment is a `workflowRuns/{runId}` doc advanced node-by-node by the QStash step worker.

- **Triggers (10)** — `contact.created`, `contact.tag.added`, `form.submitted` (optional per-form filter), `pipeline.stage.changed` (optional target-stage filter), `booking.created`, `booking.cancelled`, `booking.rescheduled`, `quote.accepted`, `quote.paid` (covers invoices), `message.received` (optional channel filter: sms/whatsapp/messenger/instagram; fired from the unified-inbox upsert). Dispatch: [src/lib/workflows/engine.ts](src/lib/workflows/engine.ts)::`fireWorkflowTrigger()` called from the server-side write paths, never from clients.
- **Nodes (14)** — send_email, send_sms, whatsapp_template (approved templates only), wait, **wait_for_reply** (v2 — see below), if_else, goal (end), add_tag, remove_tag, move_stage, update_field (allowlisted fields + customFields.*), create_task, notify (owner/account-contact/custom), webhook (POSTs contact + form fields to any URL). Registry in `engine.ts`; labels/config-shapes in [src/lib/workflows/catalog.ts](src/lib/workflows/catalog.ts).
- **wait_for_reply (v2)** — a branching node: "wait up to N for the contact's next inbound message". Arm phase parks the run (`run.waiting`, status `waiting`) and schedules a same-node QStash timeout callback carrying `phase: "timeout"`; the unified-inbox upsert ([src/lib/server/conversations-service.ts](src/lib/server/conversations-service.ts)) resolves parked runs early via `resumeWaitingRunsOnReply()`. Both paths claim the node in a transaction (re-checking `waiting` + history), so exactly one of Replied/No-reply wins; the winner's history entry makes the loser's callback a no-op via the step worker's idempotency guard. Detects SMS/WhatsApp/Messenger/Instagram only — **email replies bypass the app by design** (Reply-To → operator's inbox) and the builder says so on the node.
- **Conditions** — `ConditionGroup = { all: Condition[], match?: "all" | "any" }` ([src/lib/workflows/conditions.ts](src/lib/workflows/conditions.ts)); `match` absent = AND (every pre-v2 doc unchanged). Used by trigger filters and if_else. Ops: equals/not_equals/contains/is_set/not_set/has_tag/in_stage/source_is.
- **Re-enrollment (v2)** — `WorkflowDoc.reentry`: `every_time` (default, absent = this) | `unless_active` | `once_ever`, enforced in `fireWorkflowTrigger` via an equality-only `workflowRuns(workflowId, contactId)` query (fails open). Test enrollments bypass. The builder defaults `message.received` templates to `unless_active` — an auto-responder that re-enrolls per message is the classic footgun.
- **Step worker** — `POST /api/workflows/step` (QStash signature-verified, in `PUBLIC_PATH_PATTERNS`). Idempotent on `run.history` (nodeId presence). Sends reuse the shared compliance helpers: opt-out checks per channel, `{{unsubscribeLink}}` in emails, merge tags, `automationsPaused` kill switch checked at dispatch AND per step.
- **Starter templates (5)** — Speed-to-Lead, Appointment Confirmation, Lead Nurture, Stage-Change Follow-up, **No-reply Follow-up** (v2: message.received → wait_for_reply 2d → SMS nudge → wait_for_reply 3d → notify on re-engage / task if still silent; seeds `reentry: "unless_active"`). Seeds in [src/lib/server/workflows-service.ts](src/lib/server/workflows-service.ts); the AI assistants can create any of them via the `create_workflow` capability.
- **Indexes** — none needed for the v2 queries: `(contactId, status)` and `(workflowId, contactId)` are equality-only, served by Firestore index merging (same as the 3-equality dispatch query).
- **Local dev** — QStash needs a public callback URL. Run a cloudflared/ngrok tunnel and set `NEXT_PUBLIC_APP_URL` to it while testing workflows; timed waits + wait_for_reply timeouts won't fire without it.
...
- **AI auto-reply (Meta DM Agent v1, BUILT 2026-07-21):** the shared AI agent answers inbound Messenger/IG DMs via a fifth transport on `maybeRespondWithAi()` — ONE channel doc (`aiAgent/meta`, id `"meta"`) covers both platforms; the reply mirrors the platform the DM arrived on, and Instagram is additionally guarded by the `instagramDm` capability (Messenger-only tokens bot-reply on Messenger only). Gated by `metaAgentEnabledByAgency` (opt-in) ON TOP of the inbox gate + the `aiAgent/meta` enabled toggle + persona. Settings at AI Agents → Messenger & Instagram ([meta-channel-section.tsx](src/components/ai-agents/meta-channel-section.tsx)); dispatch at the tail of the inbound webhook (`maybeDispatchAiReply`). Inherits conversation AI controls (auto/suggest/off + human-takeover pause) for free; suggest-mode drafts approve through the existing meta send route. Plan: `META_DM_AGENT_V1_PLAN.md`.
- **NOT yet (deferred):** out-of-window sends via message tags / `HUMAN_AGENT`, multi-Page selection (connect picks the first managed Page), and rich media (text only — incl. bot replies). The hard external dependency remains **Meta App Review**; all code degrades gracefully until creds exist.
...
**Live appointment booking (Voice Booking v1, BUILT 2026-07-22).** Inbound calls can book real appointments on ONE designated booking page (`voice.bookingPageSlug`, picked in Voice settings — published, non-deposit pages only; null = off, agent falls back to the booking-link behavior). No Vapi tools/re-provisioning: because voice runs custom-LLM mode, booking is a **marker-interception loop inside the LLM webhook** — the model emits `[[slots]]` / `[[book start="…" name="…"]]`, the route intercepts before TTS, executes against [src/lib/server/voice-booking-service.ts](src/lib/server/voice-booking-service.ts) (same pure availability/host/token/lifecycle blocks as the public book route — transactional slot re-verify, so no double-book), and feeds the result back for the spoken reply (≤2 tool rounds/turn; `stripVoiceUnspeakables` is the marker backstop). Locked scope: phone-only (page email requirement + intake fields waived; NO confirmation email or reminders — both email-based), 7-day slot horizon (≤ page `visibleDays`), offers ≤3 slots verbally, inbound-only. Plan: `VOICE_BOOKING_V1_PLAN.md`.

**Data model.** `subAccounts/{id}/aiAgent/voice` carries the standard `AiChannelConfig` fields plus a nested `voice: VoiceChannelConfig` block (`greeting`, `voiceProvider`, `voiceId`, `maxCallSeconds`, `bookingPageSlug`, `vapiAssistantId`, `vapiPhoneNumberId`). The two Vapi linkage ids are server-managed — the operator can't set them via PATCH; they're populated by the provisioning round-trip when the channel is enabled and cleared when it's disabled. `subAccounts/{id}/voiceCalls/{callId}` (doc id = Vapi callId for natural retry-dedup) carries one summary doc per call: caller phone, duration, summary, ended reason, contactId, callbackRequested flag, captured fields, link to the created Task. No turn-by-turn transcript or audio per the v1 spec.
...
- `subAccountDoc.bookingLink` — the sub-account's **default booking link**: the URL the `{{bookingLink}}` merge tag resolves (workflows, broadcasts, WhatsApp templates, review requests) AND the link the AI agents share when someone wants to book (`buildSystemPrompt`'s channel-aware booking block — text channels share the URL, voice promises it by text). Set by admins via the **Default booking link** card on the Booking page ([src/components/booking/default-booking-link-card.tsx](src/components/booking/default-booking-link-card.tsx)) — pick a published booking page or paste a custom URL (Calendly etc.); saves through the existing sub-account PATCH. (The original setter died with the legacy Automations → Settings page — until this card, the tag silently resolved to "".)
...
- **Heartbeat / subscription gate**: [src/lib/gitpage/heartbeat.ts](src/lib/gitpage/heartbeat.ts) `sendHeartbeat()` checks the deployment's website-builder subscription. The response includes `gitpageStatus.agency: boolean` which gets cached at `system/gitpageStatus`. The website-builder UI reads that doc via `onSnapshot` and renders an "activate" banner when `agency === false`. Fired once per cold start via [instrumentation.ts](instrumentation.ts) and once daily via QStash → [/api/cron/gitpage-heartbeat](src/app/api/cron/gitpage-heartbeat/route.ts) (signature-verified). To set up the daily schedule: in Upstash QStash dashboard create a schedule pointing at `${NEXT_PUBLIC_APP_URL}/api/cron/gitpage-heartbeat` with cron `0 3 * * *`.
...
|---|---|---|
| `leadstack-gitpage-heartbeat` | `0 3 * * *` | Daily gitpage subscription status refresh. |
...
- **Every Public API request returns 500 `internal_error` (even with a valid key)** — missing Firestore indexes on a fresh Firebase project. API-key auth runs a `collectionGroup("apiKeys").where("prefix", "==", …)` query that needs the `apiKeys.prefix` COLLECTION_GROUP field override, and each `/api/v1/*` list endpoint (contacts / deals / tasks / events) needs its `(subAccountId, mode, <sort field>)` composite — all declared in `firestore.indexes.json`. Run `firebase deploy --only firestore:rules,firestore:indexes` and wait for the indexes to finish BUILDING (Firebase Console → Firestore → Indexes). The generic 500 is deliberate (the API never leaks internals); the real `FAILED_PRECONDITION` error is in Vercel logs under `[api/v1] handler threw`.
