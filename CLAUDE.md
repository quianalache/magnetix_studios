...
| `message_templates/{id}` | sub-account; admin-write | Reusable email + SMS bodies with merge tags. Still used by SMS automation steps and the automations "Send Email" workflow node — NOT by broadcasts since the composer rebuild |
| `broadcastTemplates/{id}` | sub-account; admin-write | User-owned broadcast composer templates — block-schema `content` + `subject`/`preheader`. **Never seeded** — created only via the composer's "Save as template," empty by default for every sub-account |
...
| `broadcasts/{id}` | sub-account read; server-only write | Bulk-email batch metadata — block-schema `content`/`subject`/`preheader`, audience filter, totals (incl. `delivered/opened/clicked/bounced/complained`), status. `templateId`/`templateName` are legacy-only, present on broadcasts sent before the composer rebuild. |
| `broadcasts/{id}/sends/{contactId}` | sub-account read; server-only write | Per-recipient delivery row. Doc id = contactId for natural dedup. Status: queued/sent/skipped/failed. Carries Resend message id + error string, plus an `engagement` object (delivered/opened/clicked/bounced/complained booleans + timestamps + counts) populated asynchronously by the Resend events webhook. |
...
| `contactLists/{id}` | server-only | Contacts redesign (2026-09-25) — saved, dynamic contact segments: `agencyId`, `subAccountId`, `name`, `description`, `group` (a `ConditionGroup` for the shared segmentation engine — never a copy of contacts), `createdByUid`/`updatedByUid`, timestamps. All access via `/api/sub-accounts/[id]/contact-lists` (tenancy-checked); also usable as a Broadcast audience (`kind: "list"`). |
...
A dedicated drag-and-drop composer (not the SMS/automation `message_templates` engine) fans out a block-schema email to a filtered audience (all contacts / a specific tag / a pipeline stage). QStash fan-out, audience resolution, and the `broadcasts`/`sends` doc shapes are unchanged from v1 — only the content model and compliance handling were rebuilt.

- **Composer**: `/sa/[id]/broadcasts/new` ([src/app/(dashboard)/sa/[subAccountId]/broadcasts/new/page.tsx](<src/app/(dashboard)/sa/[subAccountId]/broadcasts/new/page.tsx>)) — drag-and-drop block list (`@dnd-kit`; block types: text/image/video/button/divider/columns, schema in [src/types/broadcast-content.ts](src/types/broadcast-content.ts)) with a live iframe preview fed by `/api/broadcasts/render` — the preview renders the EXACT same function the real send uses, never a second approximate render. "Save as template" writes to `broadcastTemplates` (user-owned, **never seeded**); "Load template" reads from it. The Contacts page's "Send bulk email" button now just navigates here instead of opening a dialog.
- **Renderer**: [src/lib/broadcasts/render-email.ts](src/lib/broadcasts/render-email.ts) — the correctness-critical, isolated module. Table-based layout, fully inline styles, max-width 600px (Outlook can't render flexbox/grid or a `<style>` block). Video blocks always render as a linked thumbnail image, never inline playback. Text-block HTML is sanitized with an email-specific allowlist (narrower than [src/lib/community/lesson-html.ts](src/lib/community/lesson-html.ts)'s — no images/iframes/headings inside a text block).
- **Send route**: [src/app/api/broadcasts/email/send/route.ts](src/app/api/broadcasts/email/send/route.ts) takes `{content, subject, preheader, audienceFilter}` directly (no more `templateId`), checks `broadcastsEnabledByAgency` + the mailing-address compliance gate, resolves the audience via [src/lib/broadcasts/audience.ts](src/lib/broadcasts/audience.ts), creates the `broadcasts/{id}` parent doc + `sends/{contactId}` rows in 500-doc batches, and fans out to QStash at 5/sec. Hard-capped at 25,000 recipients per broadcast.
- **Step executor**: [src/app/api/broadcasts/email/step/route.ts](src/app/api/broadcasts/email/step/route.ts) is the QStash callback. Verifies signature, idempotency-checks the send row, re-checks `contact.emailOptedOut` + the sub-account's mailing address live, renders via `render-email.ts`, adds `List-Unsubscribe`/`List-Unsubscribe-Post` headers, calls `sendTenantEmail`, writes the row's status + atomically increments parent totals. Failures are recorded as `status: "failed"` and return 200 — one bounce never retry-storms the batch.
- **Compliance**: [src/lib/broadcasts/compliance.ts](src/lib/broadcasts/compliance.ts) — `requireMailingAddress()` gates every send (a required, non-optional param on the renderer, so a call site can't compile without resolving it), `buildListUnsubscribeHeaders()`/`buildBroadcastUnsubscribeHeaders()` for RFC 8058 one-click unsubscribe. The physical mailing address (`SubAccountDoc.mailingAddress`, edited in Settings → Sending preferences via [src/components/settings/sub-account-broadcast-compliance-section.tsx](src/components/settings/sub-account-broadcast-compliance-section.tsx)) + the unsubscribe link are appended automatically to every broadcast's footer by the renderer — never a block, never author-editable, never a merge tag. Per-recipient unsubscribe links resolve at `/u/[token]`, HMAC-signed with `AUTOMATIONS_TOKEN_SECRET`, flip `contact.emailOptedOut` on click. **Scope note**: this gate applies to broadcasts only — the automations "Send Email" workflow step still uses the older `{{unsubscribeLink}}`-merge-tag model and does not require a mailing address.
- **Engagement analytics + auto-suppression**: [src/lib/broadcasts/engagement-webhook.ts](src/lib/broadcasts/engagement-webhook.ts) handles `email.delivered/opened/clicked/bounced/complained` — **dispatched from the same webhook endpoint/route as inbound email** ([src/app/api/webhooks/resend/inbound/route.ts](src/app/api/webhooks/resend/inbound/route.ts), still named `inbound/` for URL stability), not a separate endpoint — Resend lets one endpoint subscribe to multiple event types, so enabling this is just checking more boxes on the existing webhook in the Resend dashboard, no new signing secret. Matches events to a `sends` row via a `collectionGroup("sends").where("resendMessageId", "==", ...)` query (see the `sends`/`resendMessageId` fieldOverride in `firestore.indexes.json`). A hard bounce or spam complaint flips `contact.emailOptedOut = true`; a soft bounce does not. Per-send `engagement` + broadcast-level `totals.{delivered,opened,clicked,bounced,complained}` power the stat tiles + per-recipient badges on the detail page.
- **List + detail pages**: `/sa/[id]/broadcasts` and `/sa/[id]/broadcasts/[broadcastId]` — onSnapshot-driven, so totals, engagement, and per-recipient status update live. Both fall back to the legacy `templateName`/`message_templates`-based fields for any broadcast sent before this rebuild.
- **No SMS broadcasts** — bulk SMS needs A2P 10DLC throughput awareness, consent-source audit fields, and TCPA-grade safeguards, explicitly deferred.
...
Improvement to the existing Contacts module (list + profile). Full report: the Build Log artifact entry for this date.

- **List — server-side pagination.** `POST /api/sub-accounts/[id]/contacts/search` returns exactly one 25-row page (`CONTACTS_PAGE_SIZE`) with search + filters + Contact List + sort evaluated together by [contacts-query-service.ts](src/lib/server/contacts-query-service.ts). Filters are the SHARED segmentation engine (`evalConditionGroup`) run in server memory over the caller's visible contacts (sub-account + territory scoped, same pattern as the Broadcast resolver); the candidate set is cached ~30s per server instance (`fresh: true` after a write; `invalidateContactsCache()` from the contact write routes). The browser no longer subscribes to every contact. Scaling limit: very large sub-accounts would want a search index. Export: `POST …/contacts/export` (same filters, ≤25k rows, CSV built client-side with the original column order first).
- **Segmentation engine extensions** ([eval-condition-group.ts](src/lib/segmentation/eval-condition-group.ts), additive — existing ops unchanged): date ops `before`/`after` (YYYY-MM-DD, whole UTC days), `within_last_days`/`more_than_days_ago`; access ops `has_access`/`not_has_access` on the pseudo-field `access` with value `offer:{id}` / `course:{id}` / `community:{groupId}`, evaluated against a server-built index ([access-index.ts](src/lib/segmentation/access-index.ts), member → contact via `members.contactId`). Without an index (client broadcast preview, workflow filters) access ops are always false — never guessed. The Workflow editor's `CONDITION_OPS` list is unchanged. Field list shared by Contacts + Broadcasts: [field-options.ts](src/lib/segmentation/field-options.ts); row editor: [condition-rows-editor.tsx](src/components/segmentation/condition-rows-editor.tsx); untrusted groups go through [sanitize-group.ts](src/lib/segmentation/sanitize-group.ts). Controlled checks: `pnpm exec tsx scripts/check-contacts-segmentation.ts`.
- **Contact Lists** = `contactLists/{id}` (see collections table). Creator or admin edits/deletes. Broadcasts: `BroadcastAudienceFilter` gained `{ kind: "list", listId }`; `resolveAudience` always loads the LIVE list (tenant-checked) and applies the usual opt-out / missing-email pre-flight; the send route stamps the resolved list name + definition onto the broadcast doc for audit; the composer's list preview comes from `POST /api/broadcasts/audience-preview`, which runs the same resolver so `confirmedAudienceSize` stays consistent. Voice-campaign audiences don't support lists yet.
- **Profile layout.** Left: `ContactInfoCard` + Energetic Decoding + Client Portal (unchanged). Center tabs: Conversations (default) · Activity · Notes. Right: collapsible `RelatedCard`s — Deals, Tasks, Projects, Submitted Forms, Purchases & Access, Quotes & invoices (each shown once; long lists capped with "view all"). Phones get a section switcher instead of three columns.
- **Conversations tab** ([contact-conversations-panel.tsx](src/components/contacts/contact-conversations-panel.tsx)) embeds the Conversations workspace's own `ActiveConversation` (`embedded` variant): merged thread, channel availability (`/api/conversations/[contactId]`), composer (reply + internal note), AI controls, pending draft, status/assignment — the same component as the inbox, so the two can't drift. The composer carries an optional email subject, an inline approved-WhatsApp-template sender (outside the 24h window) and the SMS segment counter. The old header Email/SMS dialogs and standalone SMS/WhatsApp thread cards were removed.
- **Notes** — same `contacts/{id}/notes` records (still shown as internal notes in Conversations). Writes go through `/api/contacts/[id]/notes[/noteId]`: author stamped from the session, edit = author only (keeps `createdBy`/`createdAt`, adds `updatedAt`/`updatedBy`), delete = author or admin. `addNote()` in `lib/firestore/contacts.ts` now calls that route (same signature). Rules: notes read-only for clients.
- **Activity** — `GET /api/contacts/[id]/activity` (paginated, category filter from [activity-categories.ts](src/lib/contacts/activity-categories.ts)); notes appear only as content-free "Note added" markers synthesized from note metadata. Labels in [activity-visuals.tsx](src/components/contacts/activity-visuals.tsx) (AI reply/escalate/skip, watchdog flag and opt-out rows no longer render as "Note added"). New types `purchase_completed`, `course_enrolled`, `community_access_granted`, `community_access_revoked` are written by [contact-activity.ts](src/lib/server/contact-activity.ts) from the existing purchase / enrollment / membership services (deterministic ids where retries could duplicate). Rules: activities read-only for clients. The Conversations contact panel's `activity-timeline.tsx` uses the same shared labels.
- **Purchases & Access** ([contact-access-service.ts](src/lib/server/contact-access-service.ts)) — read projection of purchases / enrollments / memberships. Admins can grant complimentary access to ANY course or community regardless of price (owner-approved): communities → membership `origin: "staff"` + a `staff:contact-grant` access source; open courses / free offers → the existing enrollment paths; PAID standalone courses → an explicit `complimentaryAccess` block on the enrollment ([StandaloneEnrollment](src/types/standalone-courses.ts)), read ONLY via `hasActiveComplimentaryAccess()` ([complimentary.ts](src/lib/standalone-courses/complimentary.ts)) — the classroom guard accepts it in place of a paid purchase and ahead of purchase-derived access windows. Never a purchase record, a payment, or a price change. Revoke ends only the grant: a paid purchase keeps access and its linked communities; a canceled subscription never strips linked-community access while a grant is active (`revokeLinked*CommunityAccessServerSide` skip). Paid Course Offers stay bundle-ungrantable (purchase-only extras) — grant their courses. A contact without a Member gets one created linked to THAT contact. Agency scope has the sibling [agency-course-complimentary-service.ts](src/lib/server/agency-course-complimentary-service.ts) (grant by email on the agency course's **Purchases & access** page; Agency Community already had owner add/remove members). E2E checks (Firestore + Auth emulators): `scripts/check-complimentary-access.ts`; rules: `scripts/check-contacts-conversations-rules.ts`.
- **Submitted Forms** — `GET /api/contacts/[id]/form-submissions` (Admin SDK collection-group query + parent-form tenancy re-check). The old client query was never authorized by the rules and hung on its skeleton.
- **Contact fields** — optional `firstName`, `lastName`, `state`, `postalCode`. `name` stays canonical and is composed from the parts only when blank ([names.ts](src/lib/contacts/names.ts)); nothing auto-splits a legacy name. Wired into create/edit, CSV import, form `mapsTo`, segmentation, columns and merge tags. The public API v1 wire format is intentionally unchanged (frozen contract).
- **Deploy notes** — deploy app code FIRST, then `firebase deploy --only firestore:rules,firestore:indexes` (the rules make notes/activities client-read-only; deploying rules first would break the old note input until the code ships). No new indexes or env vars.
