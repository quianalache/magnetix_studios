...
export async function generateMetadata({
  ...
  // Next's generated LayoutProps for this segment requires every
  // exported function here (not just the default export) to accept the
  // @modal slot's prop, required (not optional), once that parallel
  // route exists (2026-09-03) — generateMetadata never reads it, but the
  // type has to be declared to satisfy the constraint.
...
export default function CommunityGroupLayout({
  ...
  // @modal/(.)community/[postId]/page.tsx. Empty (default.tsx) on every
  // URL except a post opened via client-side navigation from within this
  // community; renders alongside `children`, never replacing it, so the
  // feed underneath stays mounted.
  modal: React.ReactNode;
}) {
  return (
    <>
