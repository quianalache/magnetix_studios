...
export default function CommunityGroupSettingsPage({
  ...
  useEffect(() => {
    return onSnapshot(
      ...
      (snap) => {
        ...
        const g = {
          id: snap.id,
          ...(snap.data() as Omit<CommunityGroup, "id">),
        };
        setGroup(g);
        setStatus(g.status);
        setAccess(g.access);
  ...
  useEffect(() => {
    const tiersQuery = query(
      collection(
        getFirebaseDb(),
        `subAccounts/${subAccountId}/communityGroups/${groupId}/tiers`
      ),
    ...
    return onSnapshot(tiersQuery, (snap) => {
      setTiers(
        snap.docs.map((d) => ({
          id: d.id,
          ...(d.data() as Omit<CommunityTier, "id">),
        }))
      );
    });
  }, [subAccountId, groupId]);
  ...
  useEffect(() => {
    const reviewsQuery = query(
      collection(
        getFirebaseDb(),
        `subAccounts/${subAccountId}/communityGroups/${groupId}/reviews`
      ),
    ...
    return onSnapshot(reviewsQuery, (snap) => {
      setReviews(
        snap.docs.map((d) => ({
          id: d.id,
          ...(d.data() as Omit<CommunityReview, "id">),
        }))
      );
    });
  }, [subAccountId, groupId]);
  ...
  if (!group) {
    return (
      <div className="text-muted-foreground mx-auto w-full max-w-3xl p-6 text-center text-sm">
        Group not found.{" "}
  ...
  function updateSidebarCard(
    index: number,
    patch: Partial<CommunitySidebarCard>
  ) {
  ...
  async function removeReview(reviewId: string) {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Couldn't remove review"
      );
    }
  }
    ...
    <div className="mx-auto w-full max-w-3xl space-y-6 p-6">
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className="hover:bg-muted inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm font-medium"
          >
      ...
      {!isAdmin && (
        <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-400">
          ...
          verified. Try refreshing the page; if this persists, sign out and back
          in.
        </p>
      )}
      ...
      <div className="bg-muted/40 text-muted-foreground rounded-lg border border-dashed p-4 text-sm">
        ...
        <p className="mt-1">
          ...
          button appears there for moderators). Group name, cover image, logo,
          and join policy are edited in{" "}
          <strong>Community Settings → General</strong> — this page no longer
          duplicates them. Public URL: <code>{publicUrl}</code>
        </p>
      </div>
      ...
      <fieldset disabled={!isAdmin || saving} className="space-y-5">
        ...
        <div className="space-y-2">
          <Label>Links</Label>
          <p className="text-muted-foreground text-xs">
            Shown in the community sidebar (e.g. your website, a resource). Up
            to 10.
          </p>
          {links.map((l, i) => (
        ...
        <div className="space-y-3 rounded-lg border p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-muted-foreground text-xs">
                ...
                the bottom of the Community Home right rail — promote an offer,
                a resource, a start-here guide, anything with an image, a short
                blurb, and one button.
              </p>
            </div>
            <Button
              type="button"
          ...
          {sidebarCards.length === 0 ? (
            <div className="text-muted-foreground rounded-md border border-dashed p-6 text-center text-sm">
              No sidebar cards yet. The Home right rail simply won&apos;t show
              this section.
            </div>
          ) : (
            <div className="space-y-3">
              {sidebarCards.map((card, i) => (
                <div
                  key={card.id}
                  className="grid gap-3 rounded-md border p-3 sm:grid-cols-[140px_1fr_auto]"
                >
                  <ImageUpload
                    label="Image"
                    value={card.imageUrl}
                  ...
                  <div className="grid gap-2">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateSidebarCard(i, { buttonUrl: e.target.value })
                        }
                        placeholder="https://…"
                      />
                    </div>
                    <ColorInput
                      ...
                      onChange={(hex) =>
                        updateSidebarCard(i, { accentColor: hex || null })
                      }
                    />
                  </div>
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setSidebarCards(sidebarCards.filter((_, j) => j !== i))
                    }
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
        ...
        {access === "paid" && (
          <div className="space-y-1.5">
            ...
            <p className="text-muted-foreground text-xs">
              ...
              access-controls slice. Until then a paid group can&apos;t be
              joined.
            </p>
          </div>
        )}
                ...
                >
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={
                        tier.priceCents != null ? tier.priceCents / 100 : ""
                      }
                      onChange={(e) =>
                        updateTier(i, {
                      ...
                      onChange={(e) =>
                        updateTier(i, {
                          billingInterval: e.target
                            .value as CommunityTier["billingInterval"],
                        })
                      }
                    >
