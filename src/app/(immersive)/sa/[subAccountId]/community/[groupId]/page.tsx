...
/**
 * Staff Community-in-CRM — the real feed. Moved (2026-09-02) out of
 * `(dashboard)` into the `(immersive)` route group so it renders full-page
 * instead of inside the CRM's Sidebar/Header/BottomTabBar — see
 * `(immersive)/sa/[subAccountId]/layout.tsx` and this route's own
 * `layout.tsx` for what replaces that chrome. Deliberately a close mirror
 * of /c/[saId]/[groupSlug]/community/page.tsx — same data fetching, same
 ...
 * (staff-shaped internal links + staff avatar/menu) with `embedded={false}`
 * (full-page presentation instead of the rounded-card one other, still-
 * embedded staff call sites keep — see CommunityShell's own doc comment).
 */
