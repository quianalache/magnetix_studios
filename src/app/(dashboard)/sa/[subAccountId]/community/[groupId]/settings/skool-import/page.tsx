...
/** Staff Community-in-CRM — Settings → Skool Import. Close mirror of
 *  /c/[saId]/[groupSlug]/settings/skool-import/page.tsx — see the Staff
 *  Community Integration report. */
export default async function StaffCommunitySkoolImportSettingsPage({
  params,
}: {
  params: Promise<{ subAccountId: string; groupId: string }>;
}) {
  const { subAccountId: saId, groupId } = await params;
  const access = await requireStaffGroupPageAccess(saId, groupId, `/sa/${saId}/community/${groupId}/settings/skool-import`);
