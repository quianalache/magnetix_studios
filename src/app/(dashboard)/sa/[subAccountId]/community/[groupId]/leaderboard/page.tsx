...
export default async function StaffLeaderboardPage({
  ...
  const access = await requireStaffGroupPageAccess(saId, groupId, `/sa/${saId}/community/${groupId}/leaderboard`);
  ...
  return (
    <CommunityShell saId={saId} group={group} active="leaderboards" viewer={viewer} viewerIsModerator={membership.role === "moderator"} staffGroupId={groupId}>
      <LeaderboardView
        brand={brand}
        viewer={viewerInfo}
