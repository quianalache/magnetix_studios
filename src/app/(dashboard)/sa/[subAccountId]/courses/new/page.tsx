...
/**
 * Full-page course-creation wizard — replaces the old cramped modal for
 * *creating* a course (editing an existing course still uses
 * `StandaloneCourseSettingsModal`). Spacious 3-step flow (Details → Upload
 * Thumbnail → Pricing) with a live preview panel that updates as you type,
 * mirroring the GoHighLevel reference Quiana walked through.
 */
export default function NewCoursePage({
  ...
  async function createCourse() {
    ...
    try {
      ...
      if (!res.ok || d.ok === false || !d.course) {
        throw new Error(d.error ?? "Couldn't create course");
      }
      toast.success("Course created.");
      router.push(`/sa/${subAccountId}/courses/${d.course.id}`);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Couldn't create course");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="mx-auto w-full max-w-6xl p-8">
      <div className="grid gap-16 lg:grid-cols-[1fr_320px]">
        <div>
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Create Course
          </Link>
          ...
          <div className="grid gap-10 sm:grid-cols-[180px_1fr]">
            ...
            <div className="max-w-xl space-y-6">
                ...
                <>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <RichTextEditor
                      ...
                      onChange={setAbout}
                      onUploadImage={(file) =>
                        uploadStandaloneCourseImage(file, subAccountId, "new", "lesson")
                      }
                    />
                    <p className="text-xs text-muted-foreground">
              ...
              {step === "thumbnail" && (
                <>
                  ...
                  <ImageUpload
                    ...
                    onUpload={(file) =>
                      uploadStandaloneCourseImage(file, subAccountId, "new", "cover")
                    }
                    aspect="video"
                  />
                </>
              ...
              <div className="flex justify-between pt-4">
                {step === "details" ? (
                  <Link href={`/sa/${subAccountId}/courses`}>
                ...
                {step === "pricing" ? (
                  <Button onClick={createCourse} disabled={saving || imgUploading}>
