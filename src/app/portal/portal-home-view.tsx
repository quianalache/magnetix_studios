...
/**
 * Shared portal-home screen — same reasoning as portal-login-view.tsx: one
 * implementation, rendered from either the opaque `/portal/{saId}` route or
 * the pretty `/portal` custom-domain mirror, so a signed-in client never
 * gets bounced off their coach's own domain mid-session. `loginPath` is the
 * one thing that differs between the two callers (which sign-in URL an
 * unauthenticated visitor gets redirected to).
 */
