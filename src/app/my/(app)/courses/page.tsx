...
export default async function MyMagnetixCoursesPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <p className="mt-1 text-[13px] text-[#84809A]">Every course you&rsquo;re enrolled in, across every Magnetix business.</p>
            ...
            return (
              ...
              >
                ...
                <Link href={course.enterHref} className="flex flex-1 flex-col">
                  <div className="relative flex h-28 items-end p-3" style={{ background: course.coverUrl ? undefined : gradientForId(key) }}>
                    {course.coverUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={course.coverUrl} alt="" className="absolute inset-0 h-full w-full object-cover" />
                    ) : (
                      <span className="relative text-[15px] font-extrabold uppercase leading-tight text-white/95">{course.title}</span>
                    )}
                  </div>
                  <div className="p-3">
