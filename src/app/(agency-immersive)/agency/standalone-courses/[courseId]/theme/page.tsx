...
/**
 * Agency Standalone Course theme editor — the exact tenant theme system
 * (same panels, same `ThemeLivePreview`), fetch-based instead of realtime
 * Firestore (agency owner has no client-readable path here, matching every
 * other agency owner surface), and `saId="agency"` threaded through so the
 * shared panels/upload helpers route through the agency Admin-SDK upload
 * route and agency theme-template API automatically.
 */
export default function AgencyCourseThemeEditorPage({
  params,
}: {
  ...
  const apiBase = "/api/agency/standalone-courses";
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/${courseId}`)
      ...
      .then((d: { course?: StandaloneCourse } | null) => {
        ...
        setTheme(d?.course?.theme ?? null);
        setLessonTheme(d?.course?.lessonTheme ?? null);
      })
      .catch(() => setCourse(null))
      .finally(() => setLoaded(true));
    fetch("/api/agency/course-offers")
      .then((r) => (r.ok ? r.json() : null))
      .then((d: { offers?: CourseOffer[] } | null) => setOtherOffers(d?.offers ?? []))
      .catch(() => {});
  }, [isOwner, courseId, apiBase]);
  ...
  async function save() {
    setSaving(true);
    try {
      const res = await fetch(`${apiBase}/${courseId}/theme`, {
        method: "PATCH",
  ...
  return (
    <div className="flex h-dvh flex-col">
      ...
      <div className="flex items-center justify-between gap-4 border-b px-4 py-3">
        <Link
          href={`/agency/standalone-courses/${courseId}`}
            ...
            <div className="p-5">
              {tab === "Layout" && page === "product" && (
                <LayoutPanel
                  ...
                  onUploadImage={(file) => uploadCourseThemeImage(file, "agency", courseId, "background")}
                  saId="agency"
                  agencyScope
              ...
              {tab === "Layout" && page === "lesson" && (
                <LayoutPanel
                  ...
                  onUploadImage={(file) =>
                    uploadCourseThemeImage(file, "agency", courseId, "lesson-background")
                  }
                  saId="agency"
                  agencyScope
                  applyTarget={{ courseId }}
              ...
              {tab === "Hero" && page === "product" && (
                <HeroPanel
                  ...
                  onUploadImage={(file) => uploadCourseThemeImage(file, "agency", courseId, "hero")}
                />
              )}
