...
interface Row extends CourseOfferPurchase {
...
/** Agency Course Offer purchases — owner view, same reasoning as the
 *  Standalone Course purchases page (read-only, Stripe-only). */
export default function AgencyCourseOfferPurchasesPage({
  params,
}: {
  ...
  useEffect(() => {
    if (!isOwner) return;
    fetch(`/api/agency/course-offers/${offerId}/purchases`)
      .then((r) => r.json())
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 p-6">
      <div>
        <Link href={`/agency/course-offers/${offerId}`} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Offer editor
        </Link>
            ...
            return (
              <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 p-3 text-sm">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{p.buyer.email} · {p.courseIds.length} course{p.courseIds.length === 1 ? "" : "s"}</div>
                </div>
