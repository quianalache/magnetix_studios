...
function parseOptions(d: Partial<PollDraft>): CommunityPollOption[] {
...
/**
 * `requireFuture` is true only when a NEW end date is being set on
 * create — rejecting a poll launched already-closed is sane there. On
 * EDIT, a stored `endsAt` can legitimately already be in the past (the
 * poll simply closed) and must stay re-savable as-is when a moderator
 * only touches `showResults`, or deliberately pushed further out to
 * reopen a closed poll — neither is "launching a backdated poll," so
 * neither should be rejected there. Only the create path passes `true`
 * (fixed before this shipped — the earlier single shared validator
 * would have made it impossible to ever re-save settings on an
 * already-closed poll without also clearing/extending its end date).
 */
function parseEndsAt(d: Partial<PollDraft>, requireFuture: boolean): CommunityPoll["endsAt"] {
  if (typeof d.endsAt !== "string" || !d.endsAt.trim()) {
    return null as unknown as CommunityPoll["endsAt"];
  }
  ...
  // Callers pass this straight through to Firestore, which accepts a real
  // Date/Timestamp — the FieldValue.serverTimestamp() cases (for
  // `createdAt`/`updatedAt`) never apply to `endsAt`, an operator-chosen
  // time, not "now".
  return parsed as unknown as CommunityPoll["endsAt"];
}

/**
 * Validate + normalize a client-supplied poll draft into stored shape for
 * CREATE — shares shape-parsing (`parseOptions`/`parseEndsAt`) with
 * `normalizePollEdit` below, same "one validator, no two call sites can
 * disagree" convention as `normalizePostAttachments`/
 * `normalizeCommentAttachments`. Returns `null` for "no poll" (not an
 * error) so a plain post's create request needs no special-casing. Throws
 * a plain `Error` (caught by the route and turned into a 400) for a
 * genuinely malformed draft — this is NOT the permission check (only a
 * moderator may submit a non-null `poll` in the first place; the route
 * checks that BEFORE calling this, since "you're not allowed to do this
 * at all" and "what you tried to do is malformed" are different,
 ...
 * this function never resets vote data. `endsAt` here is NOT required to
 * be in the future (see `parseEndsAt`'s own comment) — a closed poll's
 * settings must stay re-savable.
 */
export function normalizePollEdit(
  ...
  const d = raw as Partial<PollDraft>;
  const options = parseOptions(d);
  const draft: CommunityPoll = {
    options,
    allowMultiple: d.allowMultiple === true,
    showResults: d.showResults === true,
    endsAt: parseEndsAt(d, false),
    voterCount: 0,
    optionCounts: Object.fromEntries(options.map((o) => [o.id, 0])),
  };
  ...
  // ORDER only, never by id: `parseOptions` mints a fresh random id for
  // every option on every submission (the client never round-trips the
  // original ids), so comparing ids would spuriously reject a no-op
