...
**CURRENT PHASE:** Phase 2B — approved Magnetix builder UX restored. Built and QA'd this task; broader Phase 2 (deeper settings taxonomy, additional prebuilt sections, persistence design) not yet started or approved.
...
**IN PROGRESS:** Nothing — Phase 2B complete pending manual user aesthetic/UX QA of the real authenticated route; broader Phase 2 scope (deeper settings taxonomy, additional prebuilt sections, persistence design) not started or approved.

**KNOWN BUGS:**
- None found in Phase 2B itself (0 browser console errors across the full QA pass, after the two runtime-warning fixes noted above).
- The Insert Undo Blocker (Puck 0.23.0 corrupting `history[0]` when a controlled `<Puck>`'s `iframe`/`metadata` props are inline object literals) has a confirmed, supported-API-only fix: hoist those props to stable references. Carried forward unchanged from Phase 1/2A — standing implementation rule (§3), enforced in review, for every future controlled `<Puck>` usage.
- Puck's own `overrides` API is documented by Puck itself as "highly experimental." Phase 2B now uses five override keys (`header`, `headerActions`, `drawer`, `outline`, `fields`) — more than Phase 2A's two, a deliberate, necessary escalation per this phase's explicit instruction ("do not preserve stock Puck panels merely to avoid customization"). Every one of the five still renders Puck's own real mechanics/content underneath (drag source, tree, field inputs) — only visual chrome is custom.
- CSS custom-property overrides meant to affect Puck's canvas-iframe content (ActionBar, selection/drop indicators) must be scoped to `:root`, not a wrapper class — a wrapper-class scope silently fails to reach anything rendered inside the iframe, even with `syncHostStyles: true`. Standing constraint for any future Puck theming work, not just `magnetix-theme.css`.
...
**AWAITING USER TEST:** Manual aesthetic/UX QA of the REAL authenticated route — go to Marketing → Pages & Funnels → click **"Try New Builder"** on any page card. Confirm on arrival: the left panel is a real icon-tile Blocks library (not a plain text list), blocks can be DRAGGED onto the canvas at an exact position, Layers/Settings feel like Magnetix rather than stock Puck, and the whole shell reads as "Magnetix builder powered by Puck," not "Puck with Magnetix colors." This session could not drive that route directly (no Firebase Auth credentials available); QA was performed against a fixture-fed harness rendering the identical `MagnetixPuckEditorShell` component — strong evidence, not a substitute for the user seeing the real route with a real page.

**AWAITING USER DECISION:** Approval to begin further Phase 2 work (deeper settings-panel taxonomy, additional prebuilt sections, persistence design) once the real route is manually verified and the visual direction is confirmed approved.
