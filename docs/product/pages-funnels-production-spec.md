...
**AWAITING USER TEST:** Manual QA of the REAL authenticated route — go to Marketing → Pages & Funnels → an existing page's "⋮" menu → "Try New Builder". This session could not drive that route directly (no Firebase Auth credentials available); QA above was performed against a fixture-fed harness rendering the identical `MagnetixPuckEditorShell` component, which is strong evidence but not a substitute for seeing the real route with a real page.
