...
**IN PROGRESS:** Nothing — the Puck Persistence + Publish Foundation is complete and awaiting user QA of the real authenticated route (see AWAITING USER TEST); System B and Version History are both next-step candidates, neither started or approved.
...
**AWAITING USER TEST:** Manual QA of the REAL authenticated route — go to Marketing → Pages & Funnels → click **"Try New Builder"** on any page card. Confirm: (1) Phase 2D items — Section/Row/Column Background field (None/Color/Image/Video), Gradient editor, Preview in a new tab with no dialog; (2) System A items — selecting a Heading/Text/Button shows a "Styles" field with collapsible Typography/Spacing/(Shadow) groups that update the canvas live (font size, weight, color, alignment, opacity); selecting a Section/Row/Column shows Spacing/Border & Radius/Shadow/Responsive/Visibility groups; setting a Tablet or Mobile override in the Responsive group changes how the page looks at that width in Preview/a resized browser tab; toggling a device off in Visibility hides the element at that width without deleting it from Layers; (3) System A Closeout items — the Settings panel now shows real **General / Styles / Animations** tabs (not one flat "Styles" field) for every selectable element; Section/Hero gets a **Min Height** field and a **Full-Width Background** Yes/No toggle that visibly decouples the background from the content's max width; a Column's Styles → Responsive group shows a **Column Width** override for Tablet and Mobile that actually changes stacking at that width; unlinking Margin/Padding/Radius no longer collapses the group or jumps back to the General tab mid-edit; (4) **Puck Persistence + Publish Foundation items — the highest-priority checks now:** click **Save Draft**, confirm it shows Saving…/Saved (not stuck disabled); navigate back to Pages & Funnels and reopen the SAME page in New Builder, confirm the saved edit is still there; hard-refresh, confirm it's still there; click **Publish**, confirm a success toast; open the page's real public URL (`/p/[pageId]`) in a new tab and confirm it matches the published design; return to the editor, make another edit, **Save Draft only** (not Publish), reload the public URL again and confirm it still shows the OLDER published version, not the new edit; click Publish again and confirm the public page now updates. This session could not drive the real route directly (no Firebase Auth credentials available); QA was performed against a fixture-fed harness (UI-level only) plus direct Admin-SDK tests and real HTTP requests against temporary Firestore documents (persistence-level) — strong evidence, not a substitute for the user completing this exact click-through on a real page.

**AWAITING USER DECISION:** Approval to begin System B (Rich Text, Image/Video element depth, auto hierarchy) OR Version History (§24.12, deferred this task) — whichever the user prioritizes next, §24.22's implementation order doesn't hard-order these two against each other. Separately, still open: approval of the §24 capability baseline itself if not already given, and the Phase 2D real-route verification.

**NEXT APPROVED TASK:** None — both System B and Version History await explicit user go-ahead per this document's standing rule (§19/§21: each phase requires its own approval, not automatic continuation). See §23 above once authorized.
...
| Section/Row/Column             | Nested container model                                                            | PARTIAL         | LAUNCH    | `layout.tsx`, `config.tsx`                                              | Hierarchy, the shared Phase 2D background system, System A's shared border/radius/box-shadow/spacing/responsive/visibility, AND (this closeout) min-height and a full-width-background independent of contained max-width content are now built on all three. Still PARTIAL against the full §24.5 target (no auto hierarchy). Kept as one row — all three still share identical implementation depth. |
...
**Totals (verified by direct row count, not narrative summary — recomputed after the Puck Persistence + Publish Foundation task): 44 rows total. BUILT: 14. PARTIAL: 8. MISSING: 22. DEFERRED: 0. SKIP: 0. 14 + 8 + 22 + 0 + 0 = 44.** (Autosave and Publish moved MISSING/PARTIAL → BUILT this task; no other row's status changed.)
