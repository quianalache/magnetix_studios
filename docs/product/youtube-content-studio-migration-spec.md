...
**Phase 0** (import architecture + data rescue), **Business Brain** (shared sub-account architecture + its Settings UI), **YTCS Phase 1** (Content nav, module shell, Dashboard, project foundation, Input step for all 6 starting points), **YTCS Phase 2** (Deep Dive + Script Prompt Builder), **YTCS Phase 3A** (Create Video), **YTCS Phase 3B** (Titles + Publish), the **YTCS Final Completion Phase** (full Saved Ideas, full Video Library, YTCS Settings), **In-App Script Generation** (Generate Script inside Script Prompt Builder), **Deep Dive Dictation** (speech-to-text mic per question, replacing the old voice-note recorder), **Extra Script Notes Dictation**, the **Workflow + Script Prompt Builder Correction Pass** (resume-safe `currentStep` across all 6 steps, internal composer-style mic placement, restored 3-value Depth Preference), the **Script Output Settings + Exact-View + AI Usage Visibility Pass** (Script Output Settings UX restored to the original product's own dropdown/disclosure design, viewed-tab state moved to the URL so it's independent of workflow progress, and internal per-generation cost/duration telemetry), the **YTCS Settings Consistency Pass** (Settings' own Script Output Type control brought in line with the same dropdown pattern), the **Script + Titles AI UX Pass** (Script Prompt Builder's Generated Script/Final Script Draft/external prompt de-duplicated into a compact, height-capped layout with a 3-generation recoverable history, and Titles upgraded from a prompt-only workflow into real in-app AI title generation alongside the preserved external prompt path, later corrected so every generated title carries its own full analysis and thumbnail ideas instead of only one shared "Top Pick"), and the **Titles Polish + Project Naming Pass** (fixed a real CSS layout bug where expanding one title card stretched its collapsed row-neighbor; Save Titles now keeps the project's own name in sync with the Selected Title, with explicit manual-rename protection so a user's own naming choice is never silently overwritten; Video Library no longer shows the Selected Title twice once the project name already says it) are all **COMPLETE and live in production** at `crm.magnetixstudios.com`. **YouTube Content Studio is now a complete, end-to-end product with in-app AI script AND title generation, resume-safe step progression, exact-view refresh, and dictation**: users are no longer required to copy either assembled prompt into an external AI tool — Script Prompt Builder and Titles can now both generate directly (Sonnet 4.6 via the existing OpenRouter client), reviewable/editable, with both original copy-paste prompt workflows preserved as secondary/power-user options; Deep Dive answers and Extra Script Notes can now be dictated (browser-native, no audio ever stored) as well as typed, via an internal bottom-right mic on the composer itself; a project's `currentStep` (workflow progress) advances and persists through every one of the 6 steps, while the tab actually being VIEWED is a separate, URL-driven concept — refreshing (or using browser Back/Forward) returns to the exact tab that was open, not merely the furthest step ever reached; Depth Preference is a real 3-value choice (Detailed/Balanced/Concise) everywhere it's used, and both it and Script Output Type now use the original product's own two-dropdown-plus-help-disclosure design, consistently in both Script Prompt Builder and Settings. Only **Content Alchemy Lab** remains future/not built, per instruction. See the Phase 0/Business Brain/Business Brain UI/**YTCS Phase 1**/**YTCS Phase 2**/**YTCS Phase 3A**/**YTCS Phase 3B**/**YTCS Final Completion Phase**/**In-App Script Generation**/**Deep Dive Dictation**/**Workflow + Script Prompt Builder Correction Pass**/**Script Output Settings + Exact-View + AI Usage Visibility Pass**/**Script + Titles AI UX Pass**/**Titles Polish + Project Naming Pass** addenda (after §26) for full details.
...
3. ~~Whether "Balanced" and "Concise" Depth Preference values are real, ever-shipped options~~ — **RESOLVED 2026-09-03**: confirmed real via direct visual evidence from the original product's own screenshots (exact UI copy for all 3 values). All 3 are now selectable in both Script Prompt Builder and YTCS Settings. See the **Workflow + Script Prompt Builder Correction Pass** addendum.
...
None yet. In-App Script Generation, Deep Dive Dictation, Extra Script Notes Dictation, and the Workflow + Script Prompt Builder Correction Pass are all done and live in production — YouTube Content Studio is now complete end to end, including in-app AI script generation, resume-safe step progression, internal-mic dictation on Deep Dive and Extra Script Notes, and a fully-restored 3-value Depth Preference, except Content Alchemy Lab, which has no approval and should not start automatically. Open items carried forward: (1) a genuine interactive click-through QA pass is still owed from a real authenticated browser session — Phase 1's UI was since visually verified by the owner in production, but later phases' UI has only been QA'd via direct Firestore/logic testing plus controlled real calls, not clicked through live — the module is now ready for the owner's own end-to-end visual QA pass; (2) the Offer/CTA-for-non-offer-projects question noted above remains an open product decision, not silently resolved (Depth Preference is now resolved — see item 3 above and the Correction Pass addendum); (3) cost/usage visibility beyond raw token telemetry is explicitly deferred (no credits, no billing, no dollar-cost calculation) — a future pricing/cost-architecture decision, not silently resolved either.
...
**Depth Preference (CORRECTED 2026-09-03 — see the Workflow + Script Prompt Builder Correction Pass addendum):**

- Detailed — CONFIRMED (only value ever seen in the real export; also the only one live-tested).
- Balanced — CONFIRMED as a real, active, selectable value as of 2026-09-03, via direct visual evidence from the original product's own screenshots (its confirmed UI copy: *"Gives you enough detail to record confidently without making the draft too huge. Best when you want support without overwhelm."*). Previously marked INFERRED/not-VERIFIED after Phase 2 — that finding is now superseded by this stronger evidence.
- Concise — CONFIRMED the same way (UI copy: *"Keeps the draft lean, focused, and easy to record. Best for quick videos or confident speakers who already know what they want to say."*). Also supersedes the earlier INFERRED finding.

Detailed's confirmed UI copy: *"Gives you more depth, examples, nuance, language, and transitions. Best when you want plenty of material to cut down later."*

Only the **UI copy** for Balanced/Concise is independently confirmed — no verified prompt-instruction text for either exists in any source. `buildScriptSettingsSection()`'s Balanced/Concise branches are therefore a **disclosed implementation adaptation**: the smallest deterministic instruction text consistent with the confirmed UI copy above, not a new scripting methodology and not a guess at the original product's own (unrecovered) prompt wording. Detailed's branch is unchanged and remains the one VERIFIED-verbatim value.
...
Script Output Type: all 4 real confirmed values exposed (Full Script, Structured Recording Draft, Talking Point Outline, Hybrid Script + Talking Points). Depth Preference: at the time of this Phase 2 addendum, **only "Detailed" was exposed as selectable** — the sole value confirmed by real data or live testing at that point; "Balanced"/"Concise" stayed unresolved. **Superseded 2026-09-03** — see the **Workflow + Script Prompt Builder Correction Pass** addendum: all 3 values are now confirmed and selectable. The Script Output Type-specific descriptive paragraph in the generated prompt is only included for "Structured Recording Draft" (the one value with verbatim captured description text) — the other 3 real types get the prompt without that one extra sentence rather than a guessed description.
...
New `subAccounts/{id}/ytcs/settings` singleton doc, matching spec §20's own already-stated direction (a sibling to `ytcs/brain`). **Sub-account-wide, not per-user** — no per-user preference model exists anywhere else in Magnetix to adapt to instead, so this doesn't invent one; it's the same scope as Business Brain. Two real fields only: `defaultScriptOutputType` (all 4 real values selectable) and `defaultDepthPreference`. At the time of this Final Completion Phase, the UI only ever showed "Detailed" as a fixed, non-selectable value, matching Script Prompt Builder's own then-current treatment. **Superseded 2026-09-03** — see the **Workflow + Script Prompt Builder Correction Pass** addendum: `defaultDepthPreference` is now a real 3-value selectable dropdown here too. Applied once, at project-creation time, onto each new project's own fields (`createVideoProject` reads the settings and copies the defaults in) — verified that changing the default afterward never rewrites an already-created project's own saved value (re-verified again in the Correction Pass).
...
# Deep Dive Dictation / Speech-to-Text (2026-09-03)

## Product decision

The Deep Dive step's per-question "Record voice note" interaction is no longer the active/primary UX. Each question now shows a small dictate mic instead — speak, and the recognized text lands directly in the same editable answer field a user can also type or paste into. This supersedes the earlier voice-note-recording pattern as Deep Dive's preferred voice interaction; it does not remove or redesign the Deep Dive question sets themselves (unchanged, still verbatim from the real migrated data).

## Transcription capability — reused, not built

Audit finding: Magnetix already has a shared, already-shipped dictation capability — `src/hooks/use-dictation.ts` (`useDictation`) and `src/components/ui/dictate-button.tsx` (`DictateButton`) — in production use on two other surfaces (`src/components/ai-suite/ai-suite-chat.tsx`, `src/components/contacts/add-note-input.tsx`) before this change. **No new transcription provider, API route, or infrastructure was built or needed.** This directly satisfies the standing reuse rule (existing Magnetix implementation first) — Deep Dive is simply this capability's newest consumer, per instruction ("Deep Dive is the first consumer" of further reuse, not the origin of the abstraction itself, which already existed).

**Approach**: browser-native Web Speech API (`SpeechRecognition`/`webkitSpeechRecognition`) — free, no backend call, no provider API key, no per-minute cost. Recognized speech is delivered directly as text by the browser engine; **no audio blob, file, or buffer is ever created in application code at all** — there is nothing to "discard after transcription" because no audio ever exists client-side or server-side in the first place. This is a stronger guarantee than "temporary audio, then discard" — it's zero audio, ever, by construction. Confirms the product's cost/storage intent (no per-minute transcription charge, no Storage cost) more completely than a server-side Whisper-style pipeline would have.

**Browser support** (evaluated, not assumed): Chrome, Edge, and Safari (macOS and iOS, vendor-prefixed) support the Web Speech API; Firefox does not. `DictateButton` already handles this gracefully — it renders nothing when unsupported, so the textarea itself (typing) is unaffected; the mic control simply doesn't appear. Given this capability was already shipped and accepted in production before this task, and the owner's own standing QA preference is Chrome, this was treated as an already-settled reuse decision, not a new provider choice requiring a stop-and-report.

## Reusable dictation architecture

Unchanged — `useDictation()` (hook: browser feature-detection, recognition lifecycle, listening/error state) and `DictateButton` (small icon-only button: `{ value, onChange }`, snapshots the field's value at dictation start, appends each finalized phrase via a shared `joinText()` helper so partial typed content is never overwritten). Deep Dive reuses `DictateButton` directly, unmodified — zero changes to either file. Documented here as confirmed available for future consumers (YTCS Brain Dump, Saved Ideas, Extra Script Notes, Business Brain text areas, future Content Alchemy Lab inputs) — none of which were touched in this pass, per instruction.

## Deep Dive UX — per-question mic, one canonical answer field

**Real-data constraint reconciled, disclosed**: Deep Dive answers have only ever persisted as ONE free-text field per format (`deepDiveAnswers` for normal Deep Dive, `productOfferDeepDiveAnswers` for Product/Offer) — never one field per question. The instruction's "for every question: its own textarea" is the ideal UX framing; genuinely fragmenting persistence into N per-question fields would have meant either inventing a second answer store (explicitly forbidden) or building fragile text-splitting/reconstruction logic against real, already-unstructured historical answer text (a real data-loss risk). The implemented design: **every question gets its own small `DictateButton`, all pointed at the same one canonical answer field** — tapping the mic next to any question dictates directly into that one shared, fully-editable textarea below. This satisfies the substantive requirement (a mic per question, dictated text lands in an editable field the user can keep working in) without creating a second store or risking real historical data. The shared textarea also has its own mic for general/non-question-specific dictation.

**Removed**: the large `VoiceNoteRecorder` control and its "Record voice note" button, from both Normal and Product/Offer Deep Dive — no longer the active interaction, per instruction. **Preserved**: any project with real historical `deepDiveVoiceNotes`/`productOfferDeepDiveVoiceNotes` still shows them, read-only, via a small collapsed "Legacy voice notes" disclosure (uses the existing `VoiceNotePlayer`, unmodified) — confirmed against real data: 2 of 16 real projects (`c832488e`, `ec006b5a`) have exactly one real historical recording each under `productOfferDeepDiveVoiceNotes`, both confirmed still present and playable after this change.

## Data model

No new fields, no new collection, no new store. `deepDiveVoiceNotes`/`productOfferDeepDiveVoiceNotes` were removed from the video PATCH route's write allowlist (no active UI writes them anymore — dictation never uploads anything) — this only blocks *future writes*; `GET` is unaffected, and real historical values are confirmed untouched and still readable.

## QA result

eslint clean; `tsc --noEmit` clean (only the same pre-existing, entirely untracked, concurrent-session errors present, confirmed via `git show HEAD:<path>`). Real Firestore QA (30/30 checks): both real legacy voice-note projects confirmed unchanged; the allowlist confirmed to strip both voice-note fields from any PATCH body (proven against a disposable doc, never a real project); disposable round trips for Normal, Product Showcase, and Signature Offer Video Deep Dive all confirmed typed-then-appended text behavior (append, not replace — matches `DictateButton`'s own established `joinText` logic), survives refresh, and leaves every unrelated/sentinel field untouched; confirmed zero new Storage objects were created by any of the QA writes above (dictation never uploads audio, so there was nothing to create). All 16 real projects confirmed byte-identical before/after. **The actual browser-native speech recognition itself was not exercised in this QA pass** — it requires a real browser + real microphone input and is unchanged, already-shipped code reused verbatim from two other production surfaces; interactive QA is disclosed as pending, not claimed as done.

## Files changed

`src/components/ytcs/deep-dive-step.tsx` (per-question `DictateButton`, legacy voice-note disclosure, `VoiceNoteRecorder` removed), `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/route.ts` (PATCH allowlist: `deepDiveVoiceNotes`/`productOfferDeepDiveVoiceNotes` removed). No changes to `src/hooks/use-dictation.ts` or `src/components/ui/dictate-button.tsx` — reused exactly as they already existed.

Shipped immediately after this (same date, not separately addended at the time): **Extra Script Notes Dictation** — the exact same `DictateButton`/`useDictation` pattern applied to Script Prompt Builder's Extra Script Notes field, replacing its own "Record voice note" control; `scriptBuilderVoiceNotes` removed from the PATCH allowlist the same way, historical values preserved read-only. That gap in this document is closed retroactively by the next addendum below, which also corrects the mic's on-screen placement for both surfaces.

---

# Workflow + Script Prompt Builder Correction Pass (2026-09-03)

Three confirmed production issues, fixed in one pass. None of these are new features — all three are corrections to gaps/regressions in work already shipped earlier in this document.

## 1. `currentStep` — full 6-step audit and fix

**Root cause of "refresh returns to Input" (and, on inspection, the same class of bug on every later step):** `currentStep` was written to Firestore exactly once — at project creation, always `"Input"` — and never again by any step's Continue action except the already-fixed Input → Deep Dive transition from the prior "Continue to Deep Dive" bug-fix task. Separately, the Video Workspace page's `viewingStep` (the locally-rendered active tab) was hardcoded to `useState("Input")` and never hydrated from the loaded project at all, so even a project whose `currentStep` *was* correctly persisted still opened on Input every time the page loaded or refreshed.

**Fix, in two parts:**
1. Every remaining Continue action now writes `currentStep` to the next step name in the same atomic save as its own step content, and only calls the parent's `onContinue()` (local tab-view navigation) once that save actually resolves — never in a swallowed-error `catch`. This is the exact pattern already established and proven for Input → Deep Dive; it is now applied to the 4 transitions that either had the same "saves but doesn't advance `currentStep`" bug (Deep Dive → Script Prompt Builder, Titles → Publish) or had **no Continue action at all** (Script Prompt Builder → Create Video, Create Video → Titles — both newly added).
2. The Video Workspace page gained a hydration effect, keyed on `project?.id`, that sets `viewingStep` from `project.currentStep` once — but only when that value is a real, valid `YTCS_STEPS` entry — the first time a project loads. It never re-fires on later saves to the same project, so it cannot fight a user who has manually clicked back to an earlier tab to review or edit it.

**Explicitly preserved, per instruction:** `viewingStep` remains a resume point, not a navigation lock — every tab stays freely clickable regardless of `currentStep`, and merely viewing an earlier step never downgrades `currentStep` (clicking a tab only calls local `setViewingStep`, it never calls `onSave`/PATCH — confirmed structurally and by QA: no write occurs on backward tab navigation). Opening or refreshing a project never mutates data merely to determine which tab to show — the hydration effect only ever calls `setViewingStep`, never `onSave`.

**Steps affected**: all 5 remaining transitions — Deep Dive → Script Prompt Builder, Script Prompt Builder → Create Video, Create Video → Titles, Titles → Publish, plus the page-level hydration fix that makes the already-fixed Input → Deep Dive transition (and all the others) actually visible after a refresh.

## 2. Dictation mic placement — shared `DictationTextarea`

Extra Script Notes' mic previously sat in a separate row above the textarea, visually disconnected from the field it fed. New shared component `src/components/ui/dictation-textarea.tsx` (`DictationTextarea`) wraps the existing, unmodified `Textarea` + `DictateButton` in a `relative` container with the mic pinned `absolute bottom-1.5 right-1.5` — a ChatGPT-composer-style "this mic types into this box" placement — with `pr-10 pb-9` reserved on the textarea so typed text never runs under the icon, and `resize-none` so the browser's own native resize grip (which would otherwise sit directly under the mic in the same corner) can't fight it for the same spot.

Per instruction, Deep Dive's shared answer textarea was inspected for the same issue and found to have it too (its own label-row mic, separate from the intentional per-question mics documented in the Deep Dive Dictation addendum above) — it now uses the same `DictationTextarea`, alongside Extra Script Notes. The per-question mics next to each Deep Dive question are unchanged and remain a deliberately separate, additional interaction (dictate an answer to one specific question, still landing in the same shared box) — not a duplicate of the box's own mic.

**Explicitly unchanged**: `DictateButton`, `useDictation`, the append-not-replace `joinText()` behavior, and the browser-native `SpeechRecognition` architecture. This is a placement-only wrapper.

## 3. Depth Preference — 3 values restored

Corrected the prior INFERRED/not-VERIFIED status (Phase 2 and Final Completion addenda above) using direct visual evidence from the original product's own screenshots, which confirm exact UI copy for all 3 values — see §10's corrected Depth Preference section for the full text. Restored a `<select>` dropdown (matching the original product's own UI pattern, not a button group) with the matching description shown underneath, in both Script Prompt Builder (`DEPTH_PREFERENCES`, exported from `script-prompt-builder-step.tsx`) and YTCS Settings (`defaultDepthPreference`, now a real 3-value default for new projects — changing it never rewrites an existing project's own saved choice, re-verified by QA).

`buildScriptSettingsSection()` in `src/lib/ytcs/script-prompt.ts` gained explicit Balanced/Concise branches alongside the unchanged, VERIFIED-verbatim Detailed branch. Because no verified prompt-instruction text exists for Balanced/Concise anywhere in any source (only the UI copy is confirmed), their instruction text is a **disclosed implementation adaptation** — the smallest deterministic wording consistent with the confirmed UI copy, not an invented scripting methodology. All 3 values flow through the same, single `buildScriptPrompt()` used by both the copy-paste workflow and the in-app Generate Script call — there is no separate AI architecture for any depth value.

## QA result

Real Firestore QA (31/31 checks) against a disposable project alongside the owner's real data: all 6 `currentStep` transitions confirmed to persist and survive a simulated refresh (Input → Deep Dive → Script Prompt Builder → Create Video → Titles → Publish); backward tab viewing confirmed not to touch `currentStep`; all entered data (`rawTranscript`, `deepDiveAnswers`, `scriptBuilderExtraNotes`, `depthPreference`, `selectedTitle`) confirmed intact end to end; all 3 Depth Preference values confirmed to produce distinct, correctly-labeled instruction text via a real `buildScriptPrompt()` call; `depthPreference` PATCH-allowlist acceptance and refresh-persistence confirmed for all 3 values; Settings' `defaultDepthPreference` confirmed to flow into a new project at creation and confirmed **not** to rewrite an already-created project when changed afterward. All 16 real projects re-counted and confirmed byte-identical before and after the run — none touched. eslint clean on all 8 touched/new files; `tsc --noEmit` clean (only pre-existing, confirmed-untracked concurrent-session errors present, verified via `git status`/`git diff --stat` on each offending path — none of them mine). A full local `pnpm run build` could not complete cleanly due to a **different** concurrent session's own untracked, unrelated, broken files (`src/app/my/(app)/purchases/`, `src/lib/server/mymagnetix-billing-portal-service.ts`) failing to resolve their own imports — confirmed via `git status`/`git diff --stat` to be untracked, not in HEAD, and not touched by this task; Vercel's production build only builds committed code, so this has no effect on deployment. **Mic placement and dictation itself were not exercised in a real browser in this pass** (same disclosed limitation as the Deep Dive Dictation addendum) — interactive QA is owed from the owner's own browser session.

## Files changed

`src/app/(dashboard)/sa/[subAccountId]/youtube-studio/workspace/[videoId]/page.tsx` (viewingStep hydration effect, wired `onContinue` into Script Prompt Builder and Create Video), `src/components/ytcs/deep-dive-step.tsx` (atomic save-then-navigate Continue, `DictationTextarea`), `src/components/ytcs/script-prompt-builder-step.tsx` (atomic save-then-navigate Continue, `DEPTH_PREFERENCES`, real `depthPreference` select, `DictationTextarea`), `src/components/ytcs/create-video-step.tsx` (new Continue action), `src/components/ytcs/titles-step.tsx` (atomic save-then-navigate Continue), `src/lib/ytcs/script-prompt.ts` (Balanced/Concise instruction branches), `src/app/(dashboard)/sa/[subAccountId]/youtube-studio/settings/page.tsx` (real `defaultDepthPreference` select), and new `src/components/ui/dictation-textarea.tsx`.

**Deep Dive Dictation is complete and live in production.** Content Alchemy Lab remains untouched, per instruction.

---

# Script Output Settings + Exact-View + AI Usage Visibility Pass (2026-09-03)

Three items, scoped to correcting and instrumenting work already shipped — no new YTCS steps, no Content Alchemy Lab.

## 1. Script Output Settings UX — restored from the original product's own screenshots

**Primary visual source material**: two screenshots of the original product's own "Script Output Settings" card (Script Output Type dropdown left, Depth Preference dropdown right, each with a collapsible "what does this mean?" disclosure underneath — shown once expanded, once collapsed).

**Discrepancies found against that source, and how each was corrected:**

| In the screenshot | What this recreation had before this pass | Fix |
|---|---|---|
| Script Output Type is a real `<select>` dropdown | A row of 4 button/pill choices | Replaced with a native `<select>`, same styling convention as Depth Preference's existing dropdown |
| Both settings have a collapsible "what does this mean?" disclosure with per-value explanations | Neither setting had any help disclosure at all | Added one `Collapsible`/`CollapsibleContent`/`CollapsibleGroupTrigger` (reused, unmodified, from `@/components/ui/collapsible` — the same base-ui wrapper already used by Pages & Funnels' Style field groups) per setting, independently toggled |
| Two-column layout, Script Output Type left / Depth Preference right | Two stacked full-width sections | `grid gap-4 sm:grid-cols-2` — two columns from `sm:` up, stacked on mobile |
| Card subtitle: "Tailor the prompt to get the right kind of draft." | "Tailor the script to the right kind of draft." | Corrected to the screenshot's exact wording |

**Script Output Type's 4 disclosure explanations** (new export `SCRIPT_OUTPUT_TYPE_DESCRIPTIONS` in `script-prompt-builder-step.tsx`, using the task's own confirmed copy):

- **Full Script** — Best if you want a complete draft written in full sentences. Use this when you want something you can read, lightly edit, or use as a strong starting script.
- **Structured Recording Draft** — Best if you want strong structure, suggested lines, and detailed talking points without feeling like you have to memorize every word. This is the best default for most creators.
- **Talking Point Outline** — Best if you like to speak naturally and only need the flow, main ideas, story cues, and CTA direction. This should feel like a recording map, not a word-for-word script.
- **Hybrid Script + Talking Points** — Best if you want the hook, intro, momentum transitions, CTA, and watch-next bridge written out, while the teaching body stays in detailed talking points.

**Depth Preference's disclosure** reuses the existing `DEPTH_PREFERENCES` array unchanged (Detailed/Balanced/Concise, restored in the prior correction pass) — now also rendered as a 3-entry reference list inside the new collapsible, in addition to the always-visible one-line summary already shown under the dropdown for whichever value is currently selected.

**One deliberate, disclosed departure from the screenshot**: the screenshot's live one-line summary under the selected Depth Preference value reads slightly differently from the task's own "confirmed copy" block (e.g. the screenshot shows "Detailed gives you more material to work with. It is easier to cut down a rich draft than stretch a thin one." vs. the confirmed copy's "Gives you more depth, examples, nuance, language, and transitions..."). Only the confirmed-copy wording was independently verified for all 3 values; the screenshot's alternate short phrasing is only legible for "Detailed" (the state shown selected), with no equivalent confirmed text for Balanced/Concise. Rather than inventing unconfirmed short-form wording for two of the three values, the live summary line continues using the one verified `DEPTH_PREFERENCES` copy — this makes the live summary and the disclosure's matching entry read identically for whichever value is selected (a minor redundancy, not a functional gap), and avoids fabricating text not present in any verified source.

**Explicitly out of scope, left untouched**: YTCS Settings' own "Default Script Output Type"/"Default Depth Preference" controls (a different screen, not shown in the source screenshot, and not named in this task) still use their pre-existing button-group/plain-description pattern. Left as-is deliberately, per "restore the original UX for [this] section" + "do not redesign the entire YouTube Content Studio" — noted here as a known, intentional inconsistency rather than silently left unexplained.

## 2. Exact-view refresh — workflow progress vs. viewed step, as two separate concepts

**Root cause of "refresh returns to the wrong tab" (this time: refreshing while viewing an earlier tab snapped forward to the furthest step reached):** the prior correction pass's fix hydrated the visible tab from the project's own `currentStep` — which is correct for *first arriving* at a project, but conflates two genuinely different things once a user has manually clicked back to review an earlier step: `currentStep` is workflow **progress** (how far the project has gotten), while the tab actually being **viewed** can legitimately be an earlier one. A project progressed to Publish, then manually reviewed at Script Prompt Builder, must reopen Script Prompt Builder on refresh — not jump back to Publish merely because Publish is the furthest step ever reached.

**Fix**: the viewed tab now lives in the URL (`?step=<StepName>`), not in local React state derived from `currentStep`. `goToStep()` (in the Video Workspace page) updates the URL via `router.push` for every tab click and every successful Continue action; refresh, browser Back/Forward, and deep links all read the same URL param, so they all restore the exact tab that was open — for free, with no extra Firestore writes. The one case the URL doesn't already answer — a brand-new visit with no `?step=` yet at all — falls back to the project's own `currentStep` (the resume point) and writes it into the URL once via `router.replace` (no new history entry, and never a Firestore write — this effect only ever touches the URL).

`currentStep` itself is completely unaffected by any of this: it is still written only by a step's own Continue action, in the same atomic save as that step's content, exactly as established in the prior correction pass. Tab clicks — including clicking backward to an earlier step — never call `onSave`/PATCH at all.

`useSearchParams()` requires a Suspense boundary to avoid a static-render bailout (an established convention in this repo — see `contacts/page.tsx`'s `ImportQueryWatcher`). The page's default export is now a thin `<Suspense>` wrapper; the real body (all of the previous logic, unchanged) moved into `VideoProjectPageInner`.

## 3. AI usage / cost visibility — internal only, no customer billing

**Telemetry audit** (`ytcsScriptGenerations` collection, written by `generate-script/route.ts`): confirmed present before this pass — `subAccountId`, `videoId`, `feature`, `model`, `promptTokens`, `completionTokens`, `totalTokens`, `status` (success/failed/truncated), `generatedAt`. Confirmed **missing**: duration, cost (provider-reported or estimated), the raw `finishReason` string, and pricing-source disclosure for any estimate. All four added in this pass, additively — no existing field changed shape.

**Real generation data** (read directly from the real sub-account, not fabricated): exactly **one** completed real generation exists — `anthropic/claude-sonnet-4.6`, 3,316 prompt tokens, 2,910 completion tokens, 6,226 total, status `success`. No duration or cost was recorded for it (that instrumentation didn't exist yet at generation time). With only one real sample, no range/average is reported — a single real data point, disclosed as such, not extrapolated into a "typical" figure.

**Real bug found and fixed in the same file this pass was already touching**: `SCRIPT_GENERATION_MODEL` was hardcoded as `"anthropic/claude-sonnet-4-6"` (hyphen) — verified directly against OpenRouter's own public `GET /api/v1/models` that this is **not a real model id**; the real id is `"anthropic/claude-sonnet-4.6"` (dot). OpenRouter was silently accepting/normalizing the incorrect slug — confirmed via the one real generation's own recorded `model` field, which came back as the correct dotted id even though the hyphenated id was sent. Production was never actually broken, but relying on that undocumented leniency was a latent risk; corrected to the canonical id. This is a model-id string fix only — no prompt content, script quality, or formatting changed.

**Cost source**: OpenRouter now returns real, per-call USD cost automatically in every response's `usage.cost` field (verified against OpenRouter's own current docs — the older `usage: {include: true}` request flag is deprecated and no longer required; "the base currency is US dollars. All of the pricing on our site and API is denoted in dollars"). `callAi()` (`openrouter.ts`) now parses and returns this as `AiCompletionResult.cost` — purely additive, existing callers unaffected. The generate-script route prefers this real, provider-reported cost (`providerReportedCostUsd`) whenever present.

**Fallback estimate** (`src/lib/ytcs/script-generation-cost.ts`), used only when the provider doesn't report a cost: verified 2026-09-03 against OpenRouter's public `/api/v1/models` for `anthropic/claude-sonnet-4.6` — $3.00 / 1M input tokens, $15.00 / 1M output tokens (`0.000003`/`0.000015` per token). Applied to the one real historical generation (no provider cost recorded at the time): 3,316 × $0.000003 + 2,910 × $0.000015 ≈ **$0.054** (about 5.4¢) — an estimate, disclosed as such, tagged with its pricing source and verification date wherever it's stored.

**Internal usage record**: `ytcsScriptGenerations` docs and the project's own `generatedScriptMeta` both gained `durationMs`, `finishReason`, `providerReportedCostUsd` (preferred) or `estimatedCostUsd` + `pricingSource` + `pricingVerifiedDate` (fallback only). No customer-facing credits, limits, or billing were built — internal visibility only, per instruction.

**Aggregation helper**: `getYtcsScriptGenerationUsageSummary(subAccountId)`, new in `ytcs-service.ts` — reads all of a sub-account's `ytcsScriptGenerations` docs and returns counts by status, total tokens, and total cost (summing whichever of provider-reported/estimated cost each doc has, with `costedGenerations` disclosing how many of the total actually contributed a cost figure). A plain server function, not a route or dashboard — the smallest thing that makes "how many scripts, how many tokens, roughly how much did they cost" answerable later, per instruction not to build a reporting product.

**Is current data enough for a future tier-pricing decision?** Not yet — one real sample is enough to confirm the instrumentation works and to establish a rough per-generation order of magnitude (~5¢ on this one real, mid-size prompt), but not enough to characterize typical cost across output types, depth preferences, or prompt sizes. The next several real generations (now automatically capturing real provider cost, not an estimate) will start building that picture without any further engineering work.

## QA result

Real Firestore QA (33/33 checks) against disposable projects alongside the owner's real data: all 6 tabs confirmed independently viewable and refresh-stable regardless of `currentStep`'s own value (progressed a disposable project to Publish, then simulated viewing and refreshing at every one of the 6 tabs — each showed its own tab, never snapped to Publish); `currentStep` confirmed untouched by any tab-viewing simulation; a fresh visit with no URL step confirmed to resume from `currentStep`; all entered data confirmed intact throughout. `scriptOutputType`'s 4 values and `depthPreference`'s 3 values confirmed accepted by the PATCH allowlist and to survive refresh. `SCRIPT_OUTPUT_TYPE_DESCRIPTIONS`/`DEPTH_PREFERENCES` confirmed to carry all their expected values and copy; `SCRIPT_OUTPUT_TYPES` (Settings' own import) confirmed unchanged in shape. The cost-estimate function confirmed to match hand-computed pricing math. The route's model id confirmed corrected. `getYtcsScriptGenerationUsageSummary`'s exact aggregation logic confirmed against the real `ytcsScriptGenerations` collection (`server-only` blocks importing that file directly from a plain script outside Next's bundler, so the logic was independently re-implemented against the same real collection and cross-checked, rather than skipped). All 16 real projects re-counted and confirmed byte-identical before/after — none touched. eslint clean on all 7 touched/new files; `tsc --noEmit` clean (only the same pre-existing, confirmed-untracked, unrelated concurrent-session errors present). Local `pnpm run build` again could not complete cleanly, for the same confirmed-unrelated reason as the prior correction pass (`src/app/my/(app)/purchases/`, `src/lib/server/mymagnetix-billing-portal-service.ts` — untracked, not in HEAD, not touched here); Vercel's production build only builds committed code. **The visual result (two-column layout, disclosure open/close, mobile stacking) was not exercised in a real browser in this pass** — interactive QA is owed from the owner's own browser session, same disclosed limitation as every prior UI-only phase in this document.

## Files changed

`src/components/ytcs/script-prompt-builder-step.tsx` (Script Output Type → dropdown, both help disclosures, two-column grid, corrected subtitle), `src/app/(dashboard)/sa/[subAccountId]/youtube-studio/workspace/[videoId]/page.tsx` (URL-driven `viewingStep`, Suspense boundary, `goToStep()`), `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/generate-script/route.ts` (corrected model id, duration + cost capture, enriched telemetry write), `src/lib/comms/ai/openrouter.ts` (`cost` field parsed from `usage.cost`), `src/lib/server/ytcs-service.ts` (new `getYtcsScriptGenerationUsageSummary`), `src/types/ytcs.ts` (`generatedScriptMeta` gained `durationMs`/`providerReportedCostUsd`/`estimatedCostUsd`/`pricingSource`/`pricingVerifiedDate`), and new `src/lib/ytcs/script-generation-cost.ts`.

Content Alchemy Lab remains untouched. Generated-script quality/formatting/prompt content was not modified in this pass, per instruction — the restored Detailed/Balanced/Concise settings exist to make that a separate, comparable evaluation later.

---

# Script + Titles AI UX Pass (2026-09-09)

Two goals: de-duplicate Script Prompt Builder's now-crowded long-text layout, and upgrade Titles from a prompt-only workflow into real in-app AI generation — while preserving both external-AI prompt paths untouched. No workflow-step changes, no Content Alchemy Lab.

## Generated Script vs. Final Script Draft — the distinction, made visible again

Both fields render a full script, which had made them feel almost identical once Generated Script existed. The underlying distinction was always real and is unchanged: **Generated Script** is the AI's current working draft — editable, regenerable, disposable. **Final Script Draft** (`compiledScript`) is the one explicitly *approved* script that Create Video, Titles, and Publish actually use. What changed is only how each is *presented*:

- Generated Script stays a normal, always-visible editor, now height-capped (`max-h-[560px] overflow-y-auto` — the requested 500–600px range; `Textarea`'s own `field-sizing-content` still lets it grow naturally to fit shorter content first). Save Edits, Copy, Regenerate, and Use as Final Script Draft are unchanged.
- Final Script Draft now starts **compact** once it has content — a "Saved / Approved" status card with Copy and View/Edit, not a second full-height document. View/Edit expands the same kind of height-capped editor; Save or Collapse returns it to the compact state. A project with no Final Script Draft yet still shows a real editor immediately (nothing to be compact about) rather than an empty status card.
- "Use as Final Script Draft" and "Save Final Script" are unchanged normal save operations — **no AI call, no token spend, no new generation** — confirmed by direct code inspection: neither `useAsFinalScriptDraft()` nor `saveFinalScript()` in `script-prompt-builder-step.tsx` calls `fetch` against any `generate-*` route; both call the existing `onSave()` PATCH path only. Regenerating Generated Script never touches `compiledScript` — unchanged from before this pass, re-verified.

## Script generation history — 3-generation recoverable retention

**Storage decision**: reused the existing `ytcsScriptGenerations` telemetry collection rather than building a second one. Every generation attempt already got a lightweight metadata doc (model/tokens/cost/status); this pass adds `scriptText` (full generated text, successful/truncated attempts only) plus `scriptOutputType`/`depthPreference` to that same doc. A new field on the project itself, `activeScriptGenerationId`, tracks which generation doc the current Generated Script editor came from, so "Previous Generations" can exclude it without comparing raw text (robust to the user having edited the text since generating it).

**Retention policy**: exactly the 3 most recently retained generations per video. When a 4th is recorded, the oldest retained one has only its `scriptText` field cleared (`FieldValue.delete()`) — its telemetry metadata (model, tokens, cost, timestamp, status) is kept forever, unaffected. This was a deliberate choice over deleting the whole doc: cost/usage visibility (the prior pass's own stated purpose for this collection) stays intact for every generation ever made, while the potentially-large recoverable text itself is what's actually capped. Eviction runs as a best-effort, fire-and-forget cleanup after every successful generation (`evictOldScriptGenerations()`) — a missed eviction just means one extra retained generation until the next successful call, never data loss.

No composite Firestore index is required anywhere in this feature: every query in `ytcs-script-generations-service.ts` is a plain `videoId ==` equality filter, sorted in memory rather than via `.orderBy()` on a different field — deliberately, since this service has no way to provision a missing index from here, and per-video generation volume is always small enough that an in-memory sort is trivially cheap.

**UI**: a collapsed "Previous Generations (N)" disclosure appears only when at least one OTHER retained generation exists (the active one is never listed redundantly). Each entry shows its timestamp, Script Output Type, Depth Preference, and an "Incomplete" badge if it was truncated — with View (expand full text, height-capped), Copy, and "Use as Current" (loads that generation's text back into the active Generated Script editor via a dedicated route that never touches `compiledScript`, and re-verifies the generation doc actually belongs to this exact video before reading it).

## External script prompt — preserved

Completely unchanged: `generate-script-prompt/route.ts`, `buildScriptPrompt()`, the "Prefer your own AI tool?" collapsed-by-default section, its Copy Prompt action. The only change is a height cap (`max-h-[560px] overflow-y-auto`) on its Textarea, matching the other long-form boxes on the page, so a very large prompt no longer makes the page several screens longer.

## In-app title generation

**Reuse, not reinvention**: `buildTitlePrompt()` (title-prompt.ts) — the real, verified-verbatim title strategy (10 titles, the 9 listed types, at least 2 Search-Driven, Top 3 "Top Pick" with a 4-part explanation each, 3 thumbnail ideas) — is completely unchanged and is still exactly what the external Title Prompt Builder (`generate-title-prompt/route.ts`, unmodified) sends a user to paste into ChatGPT/Claude. A new `buildTitleGenerationPrompt()` (title-generation.ts) appends only a JSON-response-format instruction on top of that same unchanged prompt text — for the in-app AI call alone; the copy-paste prompt a user sees is byte-identical to before this pass.

**Structured output contract**: the model is asked to return one JSON object (`{ titles: [...10], topPick: {...}, thumbnailIdeas: [...3] }`) rather than freeform prose, so the UI never scrapes text. `parseTitleGenerationResponse()` validates strictly server-side — exactly 10 titles, exactly 3 with `topPickRank` 1/2/3 (no duplicates/gaps), a `topPick` object whose title matches the #1-ranked entry, exactly 3 thumbnail ideas — and throws a descriptive error on any violation. A malformed response is never saved; the route returns a clean, recoverable error instead (`generate-titles/route.ts`).

**One deliberate, disclosed adaptation**: the real captured prompt says "For each Top Pick, explain..." — i.e. all 3 Top Picks get their own explanation, not just one. The route asks for and validates all 3 titles being correctly ranked, but only the #1-ranked title's full explanation is requested/stored (`topPick`) and shown in its own "Top Pick" section, per the task's own UI guidance ("do not overload every title card with long explanation text... keep cards compact"). This is a presentation simplification, not a change to the underlying strategy.

**UI**: Generate Titles is now the primary action. Results render as compact cards (title, title type, character count, "Top N" badge where relevant) in a 2-column grid, never as raw model text in one giant textarea. Each card has "Use as Primary"/"Use as Backup", which populate `selectedTitle`/`backupTitle` — both fields stay fully, independently editable afterward, never locked. A separate compact "Top Pick" section shows the #1 title's explanation; a separate "Thumbnail Ideas" section lists the 3 ideas. Regenerating replaces the current 10-title result outright — **no title-generation history at launch**, per instruction (titles are cheap/fast enough to regenerate that a recovery feature wasn't judged worth the complexity, unlike scripts) — and never resets `selectedTitle`/`backupTitle`, since the generate handler never touches that state at all.

## External title prompt — preserved, now collapsed by default

Unchanged logic (`generate-title-prompt/route.ts`, `buildTitlePrompt()`, Copy Title Prompt). Newly wrapped in the same "Prefer your own AI tool?" collapsed-by-default pattern Script Prompt Builder already used (previously always-visible, the only Titles-step change to this path's presentation), with the same height cap on its Textarea.

## Title-generation telemetry

New `ytcsTitleGenerations` collection (`ytcs-title-generations-service.ts`), mirroring the script-generation telemetry's shape and discipline exactly (same field set: subAccountId, videoId, feature, model, promptTokens/completionTokens/totalTokens, status, finishReason, durationMs, providerReportedCostUsd/estimatedCostUsd + pricing source/date, generatedAt) but its own collection — titles have no text-retention requirement, so there was no reason to reuse the script collection's now-dual-purpose shape. `feature: "ytcs_title_generation"`. Cost estimation reuses the existing `estimateScriptGenerationCostUsd()`/`YTCS_SCRIPT_MODEL_PRICING` from the script-generation pass outright (same model family), rather than a second pricing table. Internal visibility only — no customer-facing credits, limits, or cost exposure were added.

## Backward compatibility

No field was renamed, removed, or destructively migrated. `generatedScript`, `compiledScript`, `generatedTitlePrompt`, `selectedTitle`, `backupTitle` all read/write exactly as before. All new fields (`activeScriptGenerationId`, `generatedTitles`, `titleTopPick`, `titleThumbnailIdeas`, `generatedTitlesMeta`, `generatingTitlesSince`) are additive and optional — an existing real project with none of them set simply renders its old UI (no history, no title cards) until it generates something new. All are server-managed only (written directly via `updateVideoProject`, never added to the client PATCH allowlist) — verified against real data: 16 real projects confirmed unchanged before/after this pass's QA.

## QA result

Real Firestore QA (41/41 checks): the JSON contract's parser confirmed against both a valid response and 5 distinct malformed shapes (each correctly throwing, none silently coerced); confirmed the JSON-format wrapper prepends onto — never alters — the real canonical title prompt text. Script-generation retention re-verified end to end against a disposable video: 4 real generations recorded, exactly 3 retained after the 4th, oldest correctly evicted (text cleared, telemetry metadata intact), newest-first ordering confirmed. PATCH allowlist confirmed to reject all 6 new server-managed fields while still accepting the unchanged client-editable ones. 5 real projects' pre-existing script/title fields spot-checked for compatibility. All 16 real projects re-counted and confirmed byte-identical before/after — none touched. eslint clean on all 12 touched/new files; `tsc --noEmit` clean (only the same pre-existing, confirmed-untracked, unrelated concurrent-session errors present). Local `pnpm run build` again blocked by the same confirmed-unrelated concurrent-session files as every prior pass this session; Vercel's production build only builds committed code. **The visual result (card grid, collapsed/expanded states, mobile stacking) was not exercised in a real browser in this pass** — interactive QA is owed from the owner's own browser session, same disclosed limitation as every prior UI-only phase in this document.

## Files changed

New: `src/lib/server/ytcs-script-generations-service.ts`, `src/lib/server/ytcs-title-generations-service.ts`, `src/lib/ytcs/title-generation.ts`, `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/generate-titles/route.ts`, `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/script-generations/route.ts`, `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/script-generations/[generationId]/use-as-current/route.ts`. Modified: `src/components/ytcs/script-prompt-builder-step.tsx` (layout de-duplication, history UI), `src/components/ytcs/titles-step.tsx` (in-app generation, title cards), `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/generate-script/route.ts` (history write/eviction wired in), `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/route.ts` (allowlist doc comment), `src/types/ytcs.ts` (new fields), `src/app/(dashboard)/sa/[subAccountId]/youtube-studio/workspace/[videoId]/page.tsx` (passes `subAccountId` into `TitlesStep`).

Content Alchemy Lab remains untouched. `currentStep`/`?step=` exact-view behavior was not touched by this pass — the one `page.tsx` change is a single added prop, unrelated to that logic.

Shipped immediately after this, not separately addended at the time: a correction pass giving every one of the 10 generated titles its own full analysis (why it works / viewer tension / curiosity, promise, or benefit / why it fits the script) and its own 3 thumbnail ideas, replacing the single shared "Top Pick"/"Thumbnail Ideas" sections this addendum originally shipped with. `title-generation.ts`'s JSON contract now requires all 5 of those fields on all 10 titles (previously only the #1-ranked title got an explanation, and one shared thumbnail set covered the whole batch); `MAX_OUTPUT_TOKENS`/timeouts in `generate-titles/route.ts` were raised proportionally (2000→6000, 120s/150s→180s/210s) for the larger real output. Each `TitleCard` gained its own compact, closed-by-default disclosure for this analysis instead of it living detached at the page bottom. Verified against one real end-to-end OpenRouter call (not just synthetic test data) that the real model reliably complies with the larger per-title schema within the new token budget. That gap in this document is closed retroactively by the next addendum below, which also fixes a real layout bug in that same card grid.

---

# Titles Polish + Project Naming Pass (2026-09-10)

Three corrections to the Titles step, all scoped narrowly — no redesign, no change to title-generation latency/prompt/model.

## 1. Title-card expansion layout bug

**Root cause**: the title-card grid (`grid gap-2 sm:grid-cols-2`) never set `align-items`, so it used CSS Grid's default `stretch` — every card in a row was forced to match the tallest card in that row. Expanding one card's "Why this title works" disclosure made its row's OTHER card stretch to the same height, leaving a large blank area inside the still-collapsed neighbor.

**Fix**: added `items-start` to the grid container. Each card now sizes to its own natural content height regardless of what its row neighbor is doing — the smallest possible CSS correction, no change to card markup, styling, badges, or actions.

## 2. Save Titles auto-syncs the project name

**Before**: a project's `name` was set once at creation (`"Untitled Video Project"` by default, or the Input step's first-question guess) and never touched again except by the explicit rename/pencil action. A project could carry a real, saved, chosen YouTube title while still being labeled "Untitled Video Project" everywhere in the UI.

**After**: Save Titles (and Continue to Publish, which saves the same title fields) now includes the current Selected Title as the project's `name` in the same PATCH request — no second button, no extra AI call, no title regeneration. This only happens when the project's naming is "auto-managed" (see below); it's a plain field write, verified by direct code inspection to be nothing more than the existing `onSave()`/PATCH path already used for every other title field.

**Manual rename protection**: new field `nameSource?: "auto" | "manual"` on `YtcsVideoProject`. New projects are created with `"auto"` (`createVideoProject`). The existing rename/pencil action (in both the Video Workspace project list and Video Library — two separate, near-identical implementations, both updated identically) now sets `nameSource: "manual"` in the same PATCH as the new name. Once manual, Save Titles' auto-sync (`shouldAutoSyncProjectName()` in `titles-step.tsx`) permanently refuses to overwrite the name again — manually renaming the project back to literally "Untitled Video Project" does NOT restore auto-management, per instruction ("keep manual choice authoritative"), since there's no existing UX signal that would mean "please resume auto-naming me."

**Legacy fallback** (all 16 real projects, verified — none has `nameSource` yet, since the field is new): a project with no `nameSource` at all infers safely — blank or literally "Untitled Video Project" is treated as auto-manageable (13 of the 16 real projects qualify, confirmed read-only, unchanged); any other existing name is treated as manual and is never silently overwritten (3 of the 16 real projects have real custom names and are protected this way). No mass migration was run or needed — the field is simply absent until a project's next Save Titles or rename touches it.

## 3. Video Library duplicate-title suppression

Once Save Titles keeps the project name in sync with Selected Title, the separate quoted `"{selectedTitle}"` line Video Library already showed underneath became pure duplication in the common case. Now: that line only renders when it says something the project name doesn't already say — i.e. `displayTitle` (`finalTitle || selectedTitle`) differs from the (trimmed) project name. This covers the legitimate case where a user manually renamed the project to something else — the real selected title stays visible as useful secondary context instead of disappearing. Video Workspace's own project list never showed this line at all (nothing to change there); the project header inside a workspace already reflects the synced name automatically, since it renders directly from the same `project.name` the parent page's existing `setProject(data.project)` already updates after any successful save — no separate wiring needed.

## QA result

Real Firestore QA (34/34 checks): `shouldAutoSyncProjectName()`'s logic confirmed for all 6 states (manual/auto/legacy-blank/legacy-default/legacy-undefined/legacy-custom); read-only pass over all 16 real projects confirmed the legacy inference classifies them exactly as expected (13 auto-manageable, 3 protected) without writing anything; a disposable project walked through the full real sequence — fresh auto project → Save Titles syncs the name → a second Save Titles updates it again → manual rename flips `nameSource` → a third Save Titles correctly leaves the manually-set name untouched while still saving Selected/Backup Title and Notes normally. Confirmed `nameSource` is in the client PATCH allowlist, the grid has `items-start`, both rename actions send `nameSource: "manual"`, and the Video Library suppression logic is present. `currentStep`/`?step=` exact-view logic was not touched by this pass — confirmed the workspace `[videoId]/page.tsx` file wasn't even part of the diff. All 16 real projects re-counted and confirmed byte-identical before/after — none touched. eslint clean; `tsc --noEmit` clean (only pre-existing, confirmed-untracked, unrelated concurrent-session errors, including a different concurrent session's own in-progress workflows/broadcast-email files). Local `pnpm run build` blocked by the same unrelated concurrent-session files as every prior pass this session; Vercel only builds committed code. **Visual QA (card layout with an expanded card next to a collapsed one, the synced project name appearing live in Workspace/Library/header, mobile stacking) was not exercised in a real browser in this pass** — interactive QA is owed from the owner's own browser session, same disclosed limitation as every prior UI-only phase.

## Files changed

`src/components/ytcs/titles-step.tsx` (grid `items-start` fix, `shouldAutoSyncProjectName()`, `titleFieldUpdates()` shared by Save Titles and Continue to Publish), `src/types/ytcs.ts` (`nameSource` field), `src/lib/server/ytcs-service.ts` (`createVideoProject` defaults `nameSource: "auto"`), `src/app/api/sub-accounts/[id]/ytcs/videos/[videoId]/route.ts` (`nameSource` added to the PATCH allowlist), `src/app/(dashboard)/sa/[subAccountId]/youtube-studio/workspace/page.tsx` and `.../videos/page.tsx` (both rename actions now set `nameSource: "manual"`; Video Library's redundant quoted-title line suppressed when it matches the project name).

Content Alchemy Lab untouched. Title-generation latency/model/prompt/response-structure intentionally untouched, per instruction — a separate latency diagnostic is planned next.
